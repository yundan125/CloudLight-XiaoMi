...
public partial class MainWindow : Window
{
    ...
    public MainWindow(MainViewModel viewModel, IPresenceRepository repository, PresenceMonitor monitor, PresenceDataTransferService transfer, StartupRegistrationService startup)
    {
        InitializeComponent(); DataContext = viewModel; _viewModel = viewModel; _repository = repository; _monitor = monitor; _transfer = transfer; _startup = startup;
        viewModel.OpenDeviceRequested += async (_, device) => await OpenDeviceAsync(device);
        _tray = new Forms.NotifyIcon { Icon = SystemIcons.Application, Text = "CloudLight Presence", Visible = true };
    ...
    private async Task ExitAsync() { if (_exiting) return; _exiting = true; try { if (_monitor.IsRunning) await _monitor.StopAsync("软件退出", CancellationToken.None); } catch { } _tray.Visible = false; _tray.Dispose(); Close(); System.Windows.Application.Current.Shutdown(); }
}
