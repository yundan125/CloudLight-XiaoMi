...
public sealed class MainViewModel : ObservableObject
{
    ...
    public MainViewModel(IPresenceRepository repository, ISubjectPresenceService subjects, IXiaomiPresenceSource source, PresenceMonitor monitor, JsonSettingsStore settings)
    {
    ...
    public AsyncRelayCommand LoginCommand { get; } public AsyncRelayCommand StartCommand { get; } public AsyncRelayCommand RefreshCommand { get; } public RelayCommand ShowAllCommand { get; } public RelayCommand ShowOnlineCommand { get; } public RelayCommand ShowOfflineCommand { get; }
    ...
    public int AllCount => Cards.Count; public int OnlineCount => Cards.Count(value => value.CurrentState == PresenceState.Online); public int OfflineCount => Cards.Count(value => value.CurrentState == PresenceState.Offline);
    ...
    {
        if (SelectedRouter is null) return; _currentSettings = _currentSettings with { SelectedRouterPartnerId = SelectedRouter.PartnerId }; await _settings.SaveAsync(_currentSettings, CancellationToken.None);
    ...
    public async Task RefreshCardsAsync()
    {
        ...
        values = values.OrderByDescending(value => value.CurrentState == PresenceState.Online).ThenBy(value => value.Name).ToList(); Cards.Clear(); foreach (var value in values) Cards.Add(value); CardsView.Refresh(); Raise(nameof(AllCount)); Raise(nameof(OnlineCount)); Raise(nameof(OfflineCount));
    }
    ...
    private bool FilterCard(object value) => value is PresenceCardViewModel item && (_filter == "全部" || (_filter == "在线" && item.CurrentState == PresenceState.Online) || (_filter == "离线" && item.CurrentState == PresenceState.Offline)) && (string.IsNullOrWhiteSpace(SearchText) || item.SearchText.Contains(SearchText, StringComparison.CurrentCultureIgnoreCase));
    private void SetFilter(string filter) { _filter = filter; CardsView.Refresh(); }
    private void OnStatusChanged(object? sender, MonitorStatus status) => RunOnUi(() => { CloudStatus = status.State switch { CloudConnectionState.Connected => "已连接", CloudConnectionState.Reconnecting => "正在重连", CloudConnectionState.NeedsLogin => "需要重新登录", CloudConnectionState.Paused => "已暂停", _ => "正在连接" }; if (status.LastUpdate is not null) { _lastUpdate = status.LastUpdate; Raise(nameof(LastUpdateText)); } DiagnosticMessage = status.Message ?? ""; LoginRequired = status.State == CloudConnectionState.NeedsLogin; Raise(nameof(Diagnostics)); RefreshCommand.Refresh(); return Task.CompletedTask; });
    ...
    public async Task SaveGeneralSettingsAsync(bool startWithWindows, bool startMinimized) { _currentSettings = _currentSettings with { StartWithWindows = startWithWindows, StartMinimized = startMinimized }; await _settings.SaveAsync(_currentSettings, CancellationToken.None); }
    ...
    public async Task PauseAsync() { if (_monitor.IsRunning) await _monitor.StopAsync("暂停监控", CancellationToken.None); Raise(nameof(IsMonitoring)); RefreshCommand.Refresh(); } public async Task ResumeAsync() { if (!_monitor.IsRunning && SelectedRouter is not null) await _monitor.StartAsync(SelectedRouter, CancellationToken.None); Raise(nameof(IsMonitoring)); RefreshCommand.Refresh(); } public async Task ReloadAfterImportAsync() => await RefreshCardsAsync();
}
...
public sealed class PresenceCardViewModel : ObservableObject
{
    ...
    {
        var active = value.ActiveDevice; var current = active is null ? "当前没有在线设备" : $"当前：{active.ConnectionType ?? "连接方式未知"} · {(active.Signal is null ? "-" : $"{active.Signal} dBm")}";
        ...
        return new(value.Subject.DisplayName, value.CurrentState, value.LastStateChangedAt, current, $"{value.Members.Count} 台关联设备", string.Join(' ', fields.Where(text => !string.IsNullOrWhiteSpace(text))), () => open(value.Subject)) { IsSubject = true };
    }
